...
class SnakeGame:
    def __init__(self):
        self.sound_die = js.Audio.new("data/die.wav")
        self.sound_eat = js.Audio.new("data/eat.wav")
        self.sprite_w = 16
        self.sprite_h = 16
        self.img = js.document.getElementById('Snake.png')
        self.canvas = js.document.getElementById('the-canvas')
        ...
        js.window.addEventListener('resize', create_proxy(self.resize))
        js.window.addEventListener('keydown', create_proxy(self.handle_key_press))

        self.interval_id = js.setInterval(create_proxy(self.step), 200)

    def resize(self, e=None):
        self.scale = js.window.innerWidth / (self.columns * self.sprite_w)
        new_rows = int(js.window.innerHeight // (self.scale * self.sprite_w))
        restart_game = new_rows != self.rows
        self.rows = new_rows

        ctx = self.canvas.getContext("2d")
        ctx.canvas.width = js.window.innerWidth
        ctx.canvas.height = js.window.innerHeight
    ...
    def restart(self, ctx):
        ...
        for i, name in enumerate(('head_right', 'horizontal', 'tail_right')):
            ...
            self.snake[-1].draw(name, self.scale, ctx)
    ...
    def step(self):
        ...
        if not rabbit_eaten:
            old_tail = self.snake.pop()  # del self.snake[-1]
            old_tail.draw_grass(self.scale, ctx)
            tail: SnakePart = self.snake[-1]
            tail.draw_tail(self.scale, ctx, self.snake[-2].dir)
        else:  # rabbit_eaten
